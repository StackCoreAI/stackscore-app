...
var get_checkout_session_exports = {};
__export(get_checkout_session_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(get_checkout_session_exports);
...
var handler = async (event) => {
  try {
    ...
    const stackKey = String(
      session.metadata?.stack_key || session.metadata?.stackKey || session.metadata?.planKey || "foundation"
    ).toLowerCase();
    return {
      statusCode: 200,
