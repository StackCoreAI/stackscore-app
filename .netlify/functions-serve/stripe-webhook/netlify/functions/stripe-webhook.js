...
var pool = process.env.XATA_PG_URL ? new Pool2({ connectionString: process.env.XATA_PG_URL, max: 2 }) : null;
function sha256(s = "") {
  return import_crypto.default.createHash("sha256").update(String(s).toLowerCase().trim()).digest("hex");
}
...
var handler = async (event) => {
  try {
    ...
    if (stripeEvent.type === "checkout.session.completed") {
      ...
      const stackKey = String(session?.metadata?.stack_key || "foundation").toLowerCase().trim();
