...
function extractApps(plan) {
  ...
  if (Array.isArray(plan?.plans)) return plan.plans.flatMap((p) => (Array.isArray(p?.apps) ? p.apps : []));
...
function fallbackStepsFor(app) {
  ...
  const primaryFeature = normalizeArray(app?.features)[0] || "the main reporting feature";
  return [
...
function routeSummaryText(plan) {
  return String(plan?.routing_summary || "").trim() || "We selected a route designed to organize reporting signals with minimal friction — and included fallback options so your plan stays flexible.";
}
...
function whyOverviewText(plan) {
  return String(plan?.why_overview || "").trim() || "CreditRoute selected this route based on your inputs, prioritizing reporting opportunities for your timeline, budget, and execution style.";
}

function titleLabel(planKey) {
  return titleForPlanKey(planKey).replace(/\s+CreditRoute$/i, "");
}
...
function manualCheckbox(label) {
  return `<li>□ ${label}</li>`;
}

function executionChecklistHtml() {
  return `
    <div class="rounded-md bg-black/40 border border-white/10 px-3 py-2.5">
      <div class="text-[11px] text-zinc-400 mb-1.5">Execution Checklist</div>
      <ul class="text-sm text-zinc-100/90 space-y-1 list-none pl-0">
        ${manualCheckbox("Create account")}
        ${manualCheckbox("Turn on the recommended feature")}
        ${manualCheckbox("Enable autopay or reminder, if applicable")}
        ${manualCheckbox("Confirm setup is active")}
        ${manualCheckbox("Move to the next CreditRoute step")}
      </ul>
    </div>
  `;
}
...
function checkpointHtml() {
  return `
    <div class="rounded-md bg-black/40 border border-white/10 px-3 py-2.5">
      ...
      <p class="text-sm text-zinc-100/90 mb-2">You’re ready to move forward when:</p>
      <ul class="text-sm text-zinc-100/90 space-y-1 list-none pl-0">
        ${manualCheckbox("Setup is complete")}
        ${manualCheckbox("Recommended feature is active")}
        ${manualCheckbox("You understand the next step")}
      </ul>
    </div>
  `;
}
  ...
  return `
    <div class="rounded-md bg-black/40 border border-white/10 px-3 py-2.5">
      ...
      <ul class="list-disc pl-5 text-sm text-zinc-100/90 space-y-1">
        <li>Move to the next step after the checklist and checkpoint are complete.</li>
...
function buildSidebarAppItem(app, index) {
  ...
  const featureMarkup = features.length ? `<div class="mt-2 text-[11px] text-zinc-400 uppercase tracking-wider">Use these features</div><ul class="mt-1 text-xs text-zinc-200/90 space-y-1 list-disc pl-4">${features.map((f) => `<li>${f}</li>`).join("")}</ul>` : "";
  const rerouteMarkup = reroutes.length ? `<div class="mt-2 text-[11px] text-zinc-400 uppercase tracking-wider">Fallback Options</div><ul class="mt-1 text-xs text-zinc-300 space-y-1 list-disc pl-4">${reroutes.map((r) => `<li>${r}</li>`).join("")}</ul>` : "";
  const tipMarkup = app.tip ? `<div class="mt-2 text-xs text-zinc-300"><span class="text-zinc-400">Tip:</span> ${app.tip}</div>` : "";

  body.innerHTML = `${featureMarkup}<div class="mt-2 text-[11px] text-zinc-400 uppercase tracking-wider">Start Here</div><ol class="mt-1 text-xs text-zinc-200/90 space-y-1 pl-4">${steps.map((step, i) => asActionStep(step, i)).join("")}</ol>${tipMarkup}${executionChecklistHtml()}${rerouteMarkup}`;
...
function renderRouteSummary(plan, apps) {
  ...
  if (stepsEl) stepsEl.innerHTML = routeStepsList(plan).map((step) => `<li>${step}</li>`).join("");
  const allReroutes = [...new Set(apps.flatMap((app) => normalizeArray(app.reroutes).map((v) => String(v).trim())).filter(Boolean))];
  if (reroutesWrap && reroutesList) {
    if (allReroutes.length) { reroutesWrap.style.display = "block"; reroutesList.innerHTML = allReroutes.map((item) => `<li>${item}</li>`).join(""); }
    else { reroutesWrap.style.display = "none"; reroutesList.innerHTML = ""; }
  }
...
function renderWhy(plan) {
  ...
  if (assumptions) {
    ...
    if (items.length) { assumptions.style.display = "block"; assumptions.innerHTML = items.map((item) => `<li>• ${item}</li>`).join(""); }
    else { assumptions.style.display = "none"; assumptions.innerHTML = ""; }
  }
}
...
function renderSnapshot(plan, apps, planKey) {
  ...
  if (signalsEl) signalsEl.innerHTML = computeSignals(apps).map((signal) => `<li>${signal}</li>`).join("");
  setText('[data-hook="scorecard-route"]', titleLabel(planKey));
}
...
function renderInstructionPanel(apps, index = 0) {
  ...
  if (websiteEl) { websiteEl.href = app.app_url || "#"; websiteEl.textContent = app.app_url || "—"; }
  if (whyEl) {
    ...
    whyEl.innerHTML = `<div class="space-y-2"><div><span class="text-[11px] uppercase tracking-wider text-zinc-400">Why This Step Matters</span></div><p>${featureText}</p><p>This helps you keep your CreditRoute organized around one action at a time.</p></div>`;
  }
  if (stepsEl) stepsEl.innerHTML = steps.map((step, i) => asActionStep(step, i)).join("");
  if (risksEl) {
    const items = executionInsights.length ? executionInsights : ["Setup may take 10–20 minutes, depending on verification requirements.", "Some tools may require email, bank, identity, or account verification.", "Wait for confirmation before moving to the next tool."];
  ...
  if (instructionRoot && !document.getElementById("execution-clarity-blocks")) {
    ...
    wrap.innerHTML = `${timeRequiredHtml()}${checkpointHtml()}${moveForwardHtml()}`;
...
async function fetchPlanFromServer(planKey) {
  ...
  const res = await fetch("/.netlify/functions/generate-plan", { method: "POST", headers: { "Content-Type": "application/json", Accept: "application/json" }, body: JSON.stringify({ stackKey: planKey, answers }) });
...
function initInstructionsBinder() {
  ...
  function applyData(sum) {
    ...
    const fallbacksEl = document.querySelector('[data-hook="inst-fallbacks"]');
    const fallbackSummary = fallbacksEl?.closest("details")?.querySelector("summary");
    if (websiteEl) { websiteEl.href = ds.url || "#"; websiteEl.textContent = ds.url || "—"; }
    if (whyEl) whyEl.innerHTML = `<div class="space-y-2"><div><span class="text-[11px] uppercase tracking-wider text-zinc-400">Why This Step Matters</span></div><p>Follow the activation steps below and prioritize the key feature inside this app.</p>${ds.tip ? `<p>Tip: ${ds.tip}</p>` : ""}</div>`;
    if (stepsEl) {
    ...
    if (risksEl && !risksEl.children.length) risksEl.innerHTML = `<li>Setup may take 10–20 minutes, depending on verification requirements.</li><li>Some tools may require email, bank, identity, or account verification.</li><li>Wait for confirmation before moving to the next tool.</li>`;
    const label = risksEl?.closest("div")?.querySelector("div");
    if (label) label.textContent = "What to Expect";
    if (fallbackSummary) fallbackSummary.textContent = "Fallback Options";
    if (fallbacksEl) fallbacksEl.innerHTML = fallbackOptionsHtml();
  ...
  detailsList.forEach((d) => d.addEventListener("toggle", () => { if (d.open) { document.querySelectorAll("#compose details.group").forEach((dd) => { if (dd !== d) dd.open = false; }); applyData(d.querySelector("summary")); } }));
  ...
  if (lastApp) {
    ...
    if (targetSummary) { const parent = targetSummary.closest("details.group"); if (parent) parent.open = true; applyData(targetSummary); applied = true; }
  }
  if (!applied) {
    ...
    if (firstOpen) { const parent = firstOpen.closest("details.group"); if (parent) parent.open = true; applyData(firstOpen); }
  }
}

function initLearnMore() {
  document.querySelectorAll("[data-learn-more]").forEach((btn) => btn.addEventListener("click", () => { const more = btn.parentElement?.nextElementSibling; if (more) more.classList.toggle("hidden"); }));
}
