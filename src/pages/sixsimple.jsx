...
const SixSimple = () => {
  ...
  const checklistItems = [
    { title: "Living Situation", ddescription: "Tell us how you're living so we can report rent or mortgage.",
      delay: "0.1s",
    },
  ...
  const legendData = [
    { name: "Rent", color: "bg-cyan-400" },{ name: "Subs", color: "bg-green-400" },{ name: "Utilities", color: "bg-pink-500" },
    { name: "Phone", color: "bg-purple-500" },{ name: "Budget", color: "bg-yellow-400" },{ name: "Repair", color: "bg-orange-400" }
  ];
  ...
  return (
    <div className="bg-neutral-950 text-white min-h-screen flex flex-col font-sans">
      ...
      <header className="w-full max-w-6xl mx-auto px-6 pt-6 pb-4 flex items-center justify-between">
        ...
        <button onClick={handleBackToHome} className="text-sm text-neutral-400 hover:text-white transition">← Back</button>
      </header>
          ...
          <section className="md:w-1/2">
            ...
            <ul className="space-y-6 text-lg">
              {checklistItems.map((item, index) => (
                <li key={index} className={`flex items-start transition-all duration-600 ease-out ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-3"}`} style={{ transitionDelay: isVisible ? item.delay : "0s" }}>
                  <CheckCircle className="text-green-500 w-7 h-7 mr-3 shrink-0 hover:animate-pulse cursor-pointer" strokeWidth={1.5} />
            ...
            <div className="flex justify-center items-end gap-3 h-64 w-full max-w-md">
              {barData.map((bar, index) => (
                <div key={index} className="flex flex-col items-center">
                  <div className={`${bar.color} ${bar.height} w-6 rounded-t-lg hover:scale-105 hover:drop-shadow-lg transition-all duration-200 origin-bottom ${isVisible ? "scale-y-100 opacity-100" : "scale-y-0 opacity-0"}`} style={{ transitionDelay: isVisible ? bar.delay : "0s", transitionDuration: "0.8s", transitionTimingFunction: "ease-out" }} />
