  ...
  return (
    <div className="space-y-6">
      {gains && (
        <div className="rounded-2xl border bg-white p-4 shadow-sm">
          ...
          <p className="mt-1 text-slate-600 text-sm">
            Lite {gains.lite} · Core {gains.core} · Boosted {gains.boosted} · Max {gains.max}
          </p>
        </div>
      )}
      ...
      <div className="rounded-2xl border bg-white p-4 shadow-sm">
        ...
        {selected.length === 0 ? (
          <p className="text-sm text-slate-500 mt-1">Pick a few apps to build your personalized stack.</p>
        ) : (
          <ul className="mt-2 flex flex-wrap gap-2">
            {selected.map((s, i) => (
              <li key={i} className="rounded-full border px-3 py-1 text-sm">{s.name}</li>
            ))}
          </ul>
        )}
      </div>
