...
<head>
  <meta charset="utf-8" />
  <title>CreditRoute Plan</title>
  <link rel="stylesheet" href="css/print.css" />
</head>
<body>
  <header>CreditRoute • Plan</header>
...
async function docraptorPdf({ html }) {
  const resp = await fetch('https://docraptor.com/docs', {
    ...
    body: new URLSearchParams({
      ...
      type: 'pdf',
      name: 'creditroute-plan.pdf',
      document_content: html,
      'prince_options[baseurl]': process.env.ASSET_BASEURL,
    }),
  });
...
router.post('/pdf', async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="creditroute-plan.pdf"');
    res.send(pdf);
  } catch (e) {
    console.error(e);
